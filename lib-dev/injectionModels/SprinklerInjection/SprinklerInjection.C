...
Foam::SprinklerInjection<CloudType>::SprinklerInjection
(
	...
	activeLink_(this->coeffDict().subDict("rtiCoeffs"). template lookupOrDefault<Switch>("active",false)),
    ...
    initialTemperature_( this->template getBaseProperty<scalar>("linkTemperature")),
    activationTemperature_(this->coeffDict().subDict("rtiCoeffs").template lookupOrDefault<scalar>("activationTemperature",432.0)),
    activated_(this->template getBaseProperty<bool>("activated")),
    linkTemperature_(this->template getBaseProperty<scalar>("linkTemperature")),
    activationTime_(this->template getBaseProperty<scalar>("activationTime")),
    filePtr_()
    
{
	if(activeLink_){
        ...
        if(activated_) {
            this->SOI_ = activationTime_;
        }

        if(initialTemperature_==0e0){
            // if linkTemperature not found in reactingCloud1OutputProperties then read from controlDict
            initialTemperature_ = this->coeffDict().subDict("rtiCoeffs").template lookupOrDefault<scalar>("initialTemperature",298.0);
            activated_=false;
            linkTemperature_ = initialTemperature_;
            activationTime_ = 1e6;
        }
	}
    ...
    {
        Info << nl << "SprinklerInjection cells angle dimension not right" << nl
             << exit(FatalError);
    }
    ...
    // label tetFaceI;
    // label tetPtI;
    // for(label parcelI=0;parcelI<totalParcels_;parcelI++){
    //     setPositionAndCell(parcelI,0,0.0,position,cellOwner,tetFaceI,tetPtI);
    // }
...
Foam::SprinklerInjection<CloudType>::SprinklerInjection
(
    const SprinklerInjection<CloudType>& im
    )
    :
    ...
    nActivatedSprinklers_(im.nActivatedSprinklers_),
    ...
    Tgas_(im.Tgas_),
    Ugas_(im.Ugas_),
    activeLink_(im.activeLink_),
    RTI_(im.RTI_),
    C_(im.C_),
    initialTemperature_(im.initialTemperature_),
    activationTemperature_(im.activationTemperature_),
	activated_(im.activated_),
...
template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::timeEnd() const
{
	return this->SOI_ + duration_;
}

template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::timeStart()
{
    nActivatedSprinklers_=0;
	if(activeLink_ && !activated_)
    {
        computeLinkTemperature();
    }
    else
    {
    ...
    )
    {
        ...
        this->template setBaseProperty< scalar >
            ("linkTemperature",linkTemperature_);
    }
...
template<class CloudType>
Foam::label Foam::SprinklerInjection<CloudType>::parcelsToInject
(
    const scalar time0,
    const scalar time1
    )
{
    ...
    {
        label numberparcels = 
            round((time1 - time0)*parcelsPerSecond_);
        // if(numberparcels == 0)
        //    {return numberparcels;}
        // DEBUG(numberparcels);
        if(numberparcels < totalParcels_)
        {
            return 0;
        }
        else
        {
            return totalParcels_ ;  // this needs to be modified (kvm)
        }
    }
    else
    {
...
template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::volumeToInject
(
    const scalar time0,
    const scalar time1
    )
...
template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::getElevationAngle(Random& rndGen){
...
template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::getAzimuthalAngle(Random& rndGen){
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::setParcelDirVec
(
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::setPositionAndCell
(
    const label parcelI,
    const label,
    const scalar,
    ...
    )
{
    ...
    position = position_+radiusToSprinkler_*parcelDirVec_;
    this->findCellAtPosition
        (
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::setParticleDiameter
(
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::setParticleVelocity
(
    typename CloudType::parcelType& parcel
)
{    
...
/* Called from InjectionModel::inject() in base class */
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::setProperties
(
    const label parcelI,
    const label,
    const scalar time,
...
template<class CloudType>
bool Foam::SprinklerInjection<CloudType>::fullyDescribed() const
{
    return false;
}
...
template<class CloudType>
bool Foam::SprinklerInjection<CloudType>::validInjection(const label)
{
    return true;
}

template<class CloudType>
Foam::scalar Foam::SprinklerInjection<CloudType>::setNumberOfParticles
(
    const label parcels,
    const scalar volume,
    ...
    )
{
    ...
    {
    case SprinklerInjection<CloudType>::pbMass:
    {
        
        // volumeTotal_ only used here
        scalar volume = this->volumeTotal_;
        scalar mass = this->massTotal_;
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::computeLinkTemperature()
{
 
    scalar To = initialTemperature_; 
    // scalar Tg = 408.0; 
    // scalar U = 1.0;
    // scalar dTg = 110.0;
    // scalar dt = 1.0;
    static scalar dTeOld=0.0;
    static scalar linkTemperature=To;


    // Initialize values on all procs to -GREAT;
    linkTemperature=-GREAT;
    scalar U=-GREAT;
    scalar Tg=-GREAT;

    const label ic = injectorCell_;
    if(ic > -1){
        ...
        scalar dTe = sqrt(U)/RTI_*(dTg-(1+C_/sqrt(U))*dTeOld)*deltaT+dTeOld;

        linkTemperature=initialTemperature_+dTe;
    ...
    reduce(linkTemperature, maxOp<scalar>());
    reduce(dTeOld, maxOp<scalar>());
    reduce(U, maxOp<scalar>());
    reduce(Tg, maxOp<scalar>());

    linkTemperature_ = linkTemperature;
    if(linkTemperature>activationTemperature_){
        Info << "Sprinkler Activated!\n";
        activated_ = true;
        activationTime_ = this->owner().time().value();
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::writeSprinklerHeader()
{
    ...
    rtiData << "# sprinkler" << 0 << " ";
    rtiData << position_ << endl;
    rtiData << "#t";
    rtiData << "     ";
    rtiData << "linkTemperature" << 0;
    rtiData << endl;
...
template<class CloudType>
void Foam::SprinklerInjection<CloudType>::writeSprinklerData()
{
    ...
    Info << incrIndent;
    Info << indent << "sprinkler " << 0 << " (active, link temperature) : " << activated_ << " " << linkTemperature_ << endl;
