...
namespace Foam
{
    defineTypeNameAndDebug(patchRtis, 0);
}

// * * * * * * * * * * * * * Private Member Functions  * * * * * * * * * * * //

void Foam::patchRtis::findElements(const fvMesh& mesh)
{
    ...
    if (patchI == -1)
    {
        FatalErrorIn
        (
            " Foam::patchRtis::findElements(const fvMesh&)"
        )   << " Unknown patch name "
            << patchName_ << endl
            << exit(FatalError);
    }
    ...
    if (pp.size() > 0)
    {
        ...
        forAll(rtiLocations(), rtiI)
        {
            const point sample = rtiLocations()[rtiI];
    ...
    if (debug)
    {
        Info<< "patchRtis::findElements" << " : " << endl;
...
:
    rtis(name, obr, dict, loadFromFiles)
{
    // When constructing rtis above it will have called the
    // rtis::findElements (since the virtual mechanism not yet operating).
...
void Foam::patchRtis::read(const dictionary& dict)
{
