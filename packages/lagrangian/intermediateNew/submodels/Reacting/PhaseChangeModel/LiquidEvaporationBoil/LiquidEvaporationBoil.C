...
) const
{
    ...
    // limit surface T to reasonable bounds for liquid property estimation
    scalar TsLimited = max(Ts,250.0); TsLimited = min(Ts,400.0); //kvm
    ...
    scalar rhos = ps*liquids_.W(X)/(specie::RR*Ts); // use TsLimited?
