...
tmp<fvVectorMatrix> surfaceShearForce::correct(volVectorField& U)
{
    ...
    if (0 && film.primaryMesh().foundObject<turbModel>("turbulenceModel"))
    {
        ...
        // kvm, should be using USurface here instead of U
        // kvm, mag(tauTan) is zero here
    ...
    volScalarField Cw("Cw", mu/(0.333333*(delta + d0)));
    Cw.min(5000.0);
